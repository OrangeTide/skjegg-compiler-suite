...
static void exec_group4(cf_cpu *cpu, uint16_t op)
{
    ...
    if ((op & 0xFFC0) == 0x4C40) {
        ...
        if (quot_reg == rem_reg) {
            /* DIVS.L/DIVU.L <ea>,Dq: quotient replaces the dividend */
            cpu->d[quot_reg] = quotient;
            set_flags_move(cpu, quotient, SZ_LONG);
        } else {
            /* REMS.L/REMU.L <ea>,Dr:Dq: only Dr is written. ColdFire
             * split the 68020 pair form into two instructions, so the
             * dividend in Dq survives and a compiler emits the remainder
             * and the quotient as two uses of the same register. */
            cpu->d[rem_reg] = remainder;
            set_flags_move(cpu, remainder, SZ_LONG);
        }
        return;
    }
