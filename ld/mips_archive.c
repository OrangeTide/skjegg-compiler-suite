/* mips_archive.c : ar(1) archive (.a) support for skj-ld-mips.
 *
 * The RISC-V archive reader with one change: a pulled member is parsed by
 * mips_parse_elf instead of rv_parse_elf.  The ar container itself is
 * arch-neutral, so all of the format handling below is identical.
 *
 ...
 * symbol index (member "/"); "ar rcs", ranlib, and skj-ar write it.
 *
 ...
 * mips_ld_archive_reset() clears it per link so no state survives a call.
 ****************************************************************/
...
int
mips_ld_is_archive(const char *path)
{
...
int
mips_ld_read_archive(struct linker *ld, const char *path)
{
    struct mapfile mf;
    const uint8_t *buf;
    size_t len;
    ...
    while (changed) {
        ...
        for (uint32_t i = 0; i < nsym; i++) {
            ...
            mips_parse_elf(&ld->arena, obj, copy, (size_t)msize, mname);
