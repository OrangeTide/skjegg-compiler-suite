/* mips_elf_read.c : ELF32 little-endian MIPS relocatable object reader
 *
 * The RISC-V reader with two changes: the machine id is EM_MIPS, and the
 * relocations are read from SHT_REL sections (8-byte Elf32_Rel entries with no
 * addend field; the addend lives in the relocated section and is read at link
 * time). */
...
int
mips_parse_elf(struct arena *a, struct ld_object *obj, const uint8_t *buf,
               size_t fsize, const char *path)
{
    ...
    for (i = 0; i < shnum; i++) {
        ...
        rel_off = get32(sh + 16);
        rel_size = get32(sh + 20);
        info_sec = (int)get32(sh + 28);
        count = (int)(rel_size / 8);
        ...
        for (int j = 0; j < count; j++) {
            const uint8_t *r = buf + rel_off + (uint32_t)j * 8;
            ...
            rel->addend = 0;            /* REL: the addend is read in-place */
...
int
mips_ld_read_object(struct arena *a, struct ld_object *obj, const char *path)
{
    if (mapfile_open(&obj->mf, path) != OK)
        die("cannot open %s", path);
    return mips_parse_elf(a, obj, obj->mf.data, obj->mf.len, path);
}
