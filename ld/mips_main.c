/* mips_main.c : skj-ld-mips, the MIPS I static linker driver */
...
{
    fprintf(stderr,
            "usage: skj-ld-mips [-V] [-T script.ld] [-e entry] [-f fmt]"
            " -o output input.o [...]\n"
            "\n"
            "MIPS I static linker.  Reads ELF32 little-endian EM_MIPS\n"
            "relocatable objects and writes an executable.\n"
            "\n"
            "  -f elf      ELF32 ET_EXEC based at 0x00400000 for qemu-mipsel"
            " (default)\n"
            "  -f ps-exe   PlayStation PS-EXE based at 0x80010000\n");
    exit(1);
}
...
main(int argc, char **argv)
{
    ...
    const char **inputs;
    int ninputs;
    int psexe;                  /* 0 ELF (default), 1 PS-EXE */
    struct linker ld;

    util_set_progname("skj-ld-mips");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-V") == 0) {
            ...
            if (++i >= argc) usage();
            if (strcmp(argv[i], "ps-exe") == 0)
                psexe = 1;
            else if (strcmp(argv[i], "elf") == 0)
                psexe = 0;
            else
                die("unknown output format: %s (want elf or ps-exe)", argv[i]);
    ...
    if (script_path) {
        ...
        mips_default_psx_script(&ld.arena, &ld.script);
    } else {
        mips_default_script(&ld.arena, &ld.script);
    }
    ...
    for (int i = 0; i < ninputs; i++) {
        if (mips_ld_is_archive(inputs[i])) {
            archives = realloc(archives,
                               (size_t)(narchives + 1) * sizeof(char *));
            archives[narchives++] = inputs[i];
        } else {
            mips_ld_read_object(&ld.arena, ld_add_object(&ld), inputs[i]);
        }
    }
    ...
    mips_ld_archive_reset();
    int pulled = 1;
    while (pulled) {
        pulled = 0;
        for (int i = 0; i < narchives; i++)
            pulled += mips_ld_read_archive(&ld, archives[i]);
    }
    free(archives);

    mips_ld_link(&ld);
    if (psexe)
        mips_ld_write_psexe(&ld, output_path);
    else
        mips_ld_write_exec(&ld, output_path);
